...
Page({
  ...
  onLoad() {
    ...
    this.setData({
      navBarHeight,
      list: storageUtil.getList('location')
    })
  },
  ...
  addItem() {
    ...
    const result = storageUtil.addItem('location', item)
    if (result.success) {
      this.setData({
        list: storageUtil.getList('location'),
        newItem: ''
  ...
  saveEdit() {
    ...
    if (storageUtil.updateItem('location', this.data.editIndex, value)) {
      this.setData({
        list: storageUtil.getList('location'),
        editIndex: -1,
        editValue: ''
  ...
  confirmDelete(index) {
    if (storageUtil.deleteItem('location', index)) {
      this.setData({
        list: storageUtil.getList('location')
      })
  ...
  confirmReset() {
    if (storageUtil.resetToDefault('location')) {
      this.setData({
        list: storageUtil.getList('location')
      })
  ...
  confirmClear() {
    if (storageUtil.clearAll('location')) {
      this.setData({
        list: storageUtil.getList('location')
      })
    ...
    wx.canvasToTempFilePath({
      ...
      success: (res) => {
        ...
        wx.showShareImageMenu({
          ...
          entrancePath: '/pages/location/location'
        })
      },
      fail: (err) => {
