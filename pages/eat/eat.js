...
Page({
  ...
  addItem() {
    ...
    const result = storageUtil.addItem('eat', item)
    if (result.success) {
      this.setData({
        list: storageUtil.getList('eat'),
        newItem: ''
  ...
  saveEdit() {
    ...
    if (storageUtil.updateItem('eat', this.data.editIndex, value)) {
      this.setData({
        list: storageUtil.getList('eat'),
        editIndex: -1,
        editValue: ''
  ...
  confirmDelete(index) {
    if (storageUtil.deleteItem('eat', index)) {
      this.setData({
        list: storageUtil.getList('eat')
      })
  ...
  confirmReset() {
    if (storageUtil.resetToDefault('eat')) {
      this.setData({
        list: storageUtil.getList('eat')
      })
